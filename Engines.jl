...
begin
	using PlutoUI, Dates, Gurobi, Cbc, Clp, GLPK, JuMP, Random, BenchmarkTools, DataStructures, LinearAlgebra, Plots, LightXML, PyCall, Statistics
	project_name = "Optimization engines" 
	date = Dates.Date(Dates.now())
	company = "Rappi"
	objective = "to evaluate optimization alternatives of the current approach."
...
1. The most popular, **minimize the total travel times**

$MIN \sum_{k \in RT} \sum_{(i,j) \in A} t_{ij} X_{ijk}$

2. For fast routes, **minimize the makespan** or time of the longest route.

$MINMAX (T_{R})$

3. For efficient-and fair-routes, **minimize total travel time / quantity of orders**. This would equally distribute the load (route time) and earnings (courier profit). Instead of sum: **standart deviation**.

$MIN \sum_{r \in R} \frac{T_{r}}{Q_{r}}$


4. For critical orders (s.a. black stores), **minimize the total arrival times**.


5. For busy days, **minimize the quantity of RTs** or routes.


We might apply the objectives **hierarchically**, **trade them off** or **switch** according to the state of the operation:

- First minimize the number of RTs to fulfill the orders and then minimize the total travel times using that ammount of RTs.

- Minimize the total travel time with soft time windows and add a penalty for each minute late arriving at customer' location.
...
"""
function solve_or_tools(instance::Instance, solver_params::Pair...)::Result
	py"""
	...
	return Result(routes, objective, missing)
end;
...
function mtz_model(instance::Instance, solver::Module, solver_params::Pair...)
	...
	final_sel = @views JuMP.value.(X).data
	final_arr = @views JuMP.value.(A).data
	final_un = @views JuMP.value.(U).data
	
	return Solution(final_sel, final_arr, final_un, objective_value(model), status)

end;

# ╔═╡ 11860680-75e8-11eb-1d6c-ff84cfe69975
"""
Multiobjective makespan
-----

Multiobjective: min travel times and makespan
...
"""
function makespan(instance::Instance, solver::Module, solver_params::Pair...)
	...
	@variable(model, A[V] >= 0) # arrival time
	@variable(model, U[V] >= 0) # units carried in route
	@variable(model, LR >= 0) # makespan
	
	mks = mean(time)

	@objective(model, Min, sum(time[i,j] * X[i,j] for i in V, j in V) + (LR * mks))
	
	@constraint(model, [i in D], LR >= A[i])
...
# ╔═╡ 238021be-76f5-11eb-33a7-7b27a11eeb47
# @benchmark

# ╔═╡ 2da33ef6-76d7-11eb-11df-8586bbf897af
function prueba(instance::Instance, solver::Module, solver_params::Pair...; initial = false)
	...
	if initial isa Solution
		@info("Adding initial solution")
		for i in 1:length(initial.units)
			set_start_value(A[i], initial.arrival_times[i])
			set_start_value(U[i], initial.units[i])
			for j in 1:length(initial.units)
				set_start_value(X[i,j], initial.routes[i,j])
			end
		end
	end
...
path_name = joinpath("Inputs", "solomon-1987", "C101_100.xml"); # C, RC, R
...
# ╔═╡ ccc1bd66-76da-11eb-0211-e9e3d5828acf
first = prueba(deepcopy(data), Gurobi, all_params["Gurobi"]...);
...
function optimal_routing(instance::Instance, solver::Module, solver_params::Pair...)
	...
	if string(solver) in callback_feature
		...
		return lazy_model(deepcopy(instance), solver, solver_params...)
		
	else
		
		@info("Using model with normal constraints in " * string(solver))
		
		return mtz_model(deepcopy(instance), solver, solver_params...)
		
	end	
end;

# ╔═╡ 97bc7576-76de-11eb-14ef-cf77fd031da6
second = prueba(deepcopy(data), Gurobi, all_params["Gurobi"]..., initial = first);
...
function run_single_opt(engine, data, all_params)
	...
	elseif engine isa Module
		...
		try
			s = optimal_routing(data, engine, all_params[string(engine)]...)
			sink = length(s.units)
			routes = get_routes(s.routes, sink-1, sink)
			
			return Result(routes, s.objective, s.status)
				
		catch err
			return Solution([], [], [], missing, err) 
		end
...
md"""
Run `get_info` to see statistics of a solution and `validate_solution` to run validations.
"""
...
engines = [Gurobi, "ORTools"]; # Gurobi, "ORTools", GLPK, CPLEX, Clp
...
begin
	...
	if run_experiment	
		...
		if length(sorted_engines) > 0
			...
			@info("Exporting updated graph!")
			
			savefig(export_plots, "Outputs/EnginesBenchmark.svg")	
			
			@info("Exporting updated txt!")
			
			open("./Outputs/Results.txt", "w") do file
				write(file, string(engine_metrics))
			end
...
# ╔═╡ 80168ba0-7580-11eb-0d3d-a773675a5867
function delete_by_index!(index, engine_metrics)
	for solver in keys(engine_metrics)
		for metric in keys(engine_metrics[solver])
			deleteat!(engine_metrics[solver][metric], index)
		end
	end
	return engine_metrics
end;

# ╔═╡ 59dac5d6-7581-11eb-057a-93ebdcd98832
# delete_by_index!(9, engine_metrics)
...
# ╔═╡ 6b8ef58c-75f8-11eb-2801-8fe584c686c7
md"""
Comparison: $(@bind boo_compare PlutoUI.CheckBox(false))
"""

# ╔═╡ 43eada74-7548-11eb-38ed-2ba5f5bdaf94
if boo_compare && length(engine_metrics["Gurobi"]) > 0
	t_mean, t_min, ov_g = compare_two("Gurobi", "ORTools", engine_metrics);
else
	t_mean, t_min, ov_g = missing, missing, missing
end;

# ╔═╡ 55480134-7552-11eb-1c9f-2582fbd05b59
md"""
## Takeaways

Across all experiments:

- Gurobi runs in $(round(t_mean; digits=2)) % less mean time,
- Gurobi runs in $(round(t_min; digits=2)) % less min time,
- Gurobi has $(round(ov_g; digits=2)) % less routing time.


Moreover, formulating the problem would give us the **flexibility** to:
- switch between different objectives depending on the state of the operation,
- reduce routing times (solutions),
- increase the scope: number of stores.

See `CVRPTW -> Multiple objectives` section.

"""

# ╔═╡ 1639c5de-75ea-11eb-304f-ed75221894ca
md"""
Execute a multiobjective: $(@bind multiobjective PlutoUI.CheckBox(false))
"""

# ╔═╡ 41161c1c-75ea-11eb-1ddc-d3dd947ff604
md"""
For instance, we've got 2 more routes compared to `mtz_model` if want to **minimize makespan AND total travel times**.
"""

# ╔═╡ 06678c18-75ea-11eb-1711-7bbbd176c1e8
multiobjective && makespan(deepcopy(data), Gurobi, all_params["Gurobi"]...)

# ╔═╡ 3e1f7066-75eb-11eb-17fb-37ff9f02f48f
function total_routing_time(route, times)
	
	cum_time = 0
	
	for i in 1:length(route)-1
		cum_time += times[route[i], route[i+1]]
	end
	
	return cum_time
end;

# ╔═╡ a0127440-75ea-11eb-330c-55c26cc9d0d5
get_info(routes, f, t) = f([total_routing_time(route, t) for route in routes]);

# ╔═╡ 5ffa23d2-75ed-11eb-161c-61dbc58c04bb
md"""
Run `get_info` to see statistics of a solution and `validate_solution` to run validations.
"""
...
1. Callbacks: implement lazy constraints to handle the exponential number of time windows and subtour elimination constraints. 

2. Branch-and-Price framework: reformulate as a Elementary Shortest Path Problem with Resource Constraints (ESPPRC) with a column generation algorithm to handle the exponential number of variables.

3. Add bigger datasets such as [Homberger](https://www.sintef.no/projectweb/top/vrptw/homberger-benchmark/) and [Russell](https://neo.lcc.uma.es/vrp/vrp-instances/capacitated-vrp-with-time-windows-instances/).

4. Heuristic algorithms. Checkout [Nodal.jl](https://github.com/phrb/NODAL.jl) and Engine.jl (local).

5. Plot solutions.

6. [Nextmv](https://docs.nextmv.io/latest/)
...
# ╔═╡ 200f1830-7558-11eb-1f65-611923503293
md"""
**References**

- Desaulniers, G., Madsen, O. B., & Ropke, S. (2014). Chapter 5: The vehicle routing problem with time windows. In Vehicle Routing: Problems, Methods, and Applications, Second Edition (pp. 119-159). Society for Industrial and Applied Mathematics.
